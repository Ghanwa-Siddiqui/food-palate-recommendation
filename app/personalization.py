...
def _weighted_sum(vectors_with_weights: list[tuple[list[float], float]]) -> list[float]:
    ...
    stacked = np.stack(
        [np.array(v, dtype=np.float32) * (w / total_w) for v, w in active]
    )
...
def build_taste_vector(answers: OnboardingAnswers) -> list[float]:
    ...
    taste_dims_vec = embed_terms(
        [
            ...
            _level_phrase("richness", answers.richness_preference),
        ]
    )
...
def user_from_onboarding(user_id: str, answers: OnboardingAnswers) -> UserTaste:
    return UserTaste(
        ...
        richness_preference=answers.richness_preference,
        ...
        city=answers.city,
        require_halal=answers.require_halal,
